...
void Testbed::reset_training_step() {
	m_training_step = 0;
}

void Testbed::reset_loss() {
...
void Testbed::imgui() {
	...
	if (ImGui::CollapsingHeader("Training", m_training_data_available ? ImGuiTreeNodeFlags_DefaultOpen : 0)) {
		...
		if (m_testbed_mode == ETestbedMode::Nerf && ImGui::TreeNode("NeRF training options")) {
			...
			ImGui::SliderFloat("Mask supervision strength", &m_nerf.training.mask_supervision_lambda, 0.f, 10.f);
	...
	if (ImGui::CollapsingHeader("Rendering", ImGuiTreeNodeFlags_DefaultOpen)) {
		...
		if (ImGui::TreeNode("Debug visualization")) {
			...
			if (m_testbed_mode == ETestbedMode::Nerf) {
				...
				ImGui::PlotLines("Training view error (pmf)", m_nerf.training.error_map.pmf_img_cpu.data(), m_nerf.training.error_map.pmf_img_cpu.size(), 0, nullptr, 0.0f, FLT_MAX, ImVec2(0, 60.f));

				ImGui::PlotLines("Training view error (abs)", m_nerf.training.error_map.abs_img_cpu.data(), m_nerf.training.error_map.abs_img_cpu.size(), 0, nullptr, 0.0f, FLT_MAX, ImVec2(0, 60.f));

				if (m_nerf.training.train_cam_with_mask_loss){
					ImGui::PlotLines("Training view error (rgb & abs)", m_nerf.training.error_map_rgb.abs_img_cpu.data(), m_nerf.training.error_map_rgb.abs_img_cpu.size(), 0, nullptr, 0.0f, FLT_MAX, ImVec2(0, 60.f));
					ImGui::PlotLines("Training view error (mask & abs)", m_nerf.training.error_map_mask.abs_img_cpu.data(), m_nerf.training.error_map_mask.abs_img_cpu.size(), 0, nullptr, 0.0f, FLT_MAX, ImVec2(0, 60.f));
				}
...
void Testbed::visualize_nerf_cameras(ImDrawList* list, const Matrix<float, 4, 4>& world2proj, int* camera_idx_list) {
	/*
	...
	/*Clearer view*/
	if (camera_idx_list==nullptr){
		n_visualization_camera_idx_list =  m_nerf.training.n_images_for_training;
	}

	int i;
	for (int camera_idx = 0; camera_idx < n_visualization_camera_idx_list; ++camera_idx) {

		if (camera_idx_list==nullptr){
			i = camera_idx;
		}
		else{
			i = camera_idx_list[camera_idx];
		}
		...
		// visualize_nerf_camera(list, world2proj, m_nerf.training.dataset.xforms[i].start, aspect, 0x40ffff40);
		// visualize_nerf_camera(list, world2proj, m_nerf.training.dataset.xforms[i].end, aspect, 0x40ffff40);
		visualize_nerf_camera(list, world2proj, current_xform, aspect, 0xffff40ff);

		// Connect the start and end camera
		// add_debug_line(list, world2proj, m_nerf.training.dataset.xforms[i].start.col(3), m_nerf.training.transforms[i].start.col(3), 0xffff40ff); // 1% loss change offset
...
void Testbed::draw_visualizations(ImDrawList* list, const Matrix<float, 3, 4>& camera_matrix, int* camera_idx_list) {
	...
	if (m_testbed_mode != ETestbedMode::Image) {
		...
		if (m_testbed_mode == ETestbedMode::Nerf) {
			if (m_nerf.visualize_cameras) {
				...
				visualize_nerf_cameras(list, world2proj, camera_idx_list);
			}
		}
...
void Testbed::draw_gui() {
	...
	if (m_single_view) {
		...
		draw_visualizations(list, m_smoothed_camera, visualization_camera_idx_list);
	}
...
void Testbed::reset_network(bool clear_density_grid) {
	...
	m_nerf.training.error_map_rgb.is_cdf_valid = false;
	m_nerf.training.error_map_mask.is_cdf_valid = false;
