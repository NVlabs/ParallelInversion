...
__global__ void copy_mask(const uint64_t num_elements, float* __restrict__ mask_dst, const T* __restrict__ mask_pixels) {
	...
	if (mask_pixels == nullptr) {
		mask_dst[i] = 0.f; // no mask data for this entire image. zero it out
	}else {
...
template <typename T>
__global__ void copy_maskindex(const uint64_t num_elements, uint16_t* __restrict__ maskinedx_dst, const T* __restrict__ maskindex_pixels) {
	const uint64_t i = threadIdx.x + blockIdx.x * blockDim.x;
	if (i >= num_elements) return;

	if (maskindex_pixels == nullptr) {
		maskinedx_dst[i] = 0; // no mask data for this entire image. zero it out
	}else {
		maskinedx_dst[i] = maskindex_pixels[i];
	}
}
...
NerfDataset create_empty_nerf_dataset(size_t n_images, int aabb_scale, bool is_hdr) {
	...
	result.maskindexmemory.resize(n_images);
...
NerfDataset load_nerf(const std::vector<filesystem::path>& jsonpaths, float sharpness_threshold, float sharpen_amount) {
	if (jsonpaths.empty()) {
	...
	struct LoadedImageInfo {
		...
		uint16_t *mask_pixels = nullptr;
		uint16_t *maskindex_pixels = nullptr;
		uint32_t maskindex_len = 0;

		bool skip_load_image = false;
	};
	...
	for (size_t i = 0; i < jsons.size(); ++i) {
		...
		// Skip loading the images if the json file has the flag
		if (json.contains("skip_load_image") && json["skip_load_image"] == true) {

			//tlog::info() << "FIND SKIP_LOAD_IMAGE";
			info.skip_load_image = true;
		}
		...
		float sharpness_discard_threshold = json.value("sharpness_discard_threshold", sharpness_threshold); // Keep all by default

		// Sort according to file_path name, do not enable it when we want to preserve the order of the files
		if (info.skip_load_image == false) {
		...
		// only makes sense when the size of the frames is larger than 1, otherwise it will be deleted indeliberately
		if (frames[0].contains("sharpness") && frames.size() > 1) {
	...
	result.maskindexmemory.resize(result.n_images);
		...
		if (json.contains("frames") && json["frames"].is_array()) pool.parallelForAsync<size_t>(0, json["frames"].size(), [&, basepath, image_idx, info](size_t i) {
			...
			if (equals_case_insensitive(path.extension(), "exr")) {
				...
				if (info.skip_load_image == false){
					...
					// tlog::info() << "Skip loading";
					// Basic idea is to skip loading for all the images.
					dst.pixels = nullptr;
					dst.image_type = EImageDataType::None;
				}
			}

			if (dst.skip_load_image == false && !dst.pixels) {
			...
			if (enable_depth_loading && info.depth_scale > 0.f && frame.contains("depth_path")) {
 			...
 			// We do not have to load the mask image directly from the json file for now. We simply use the set_image python function later.
	...
	for (uint32_t i = 0; i < result.n_images; ++i) {
		...
		result.set_training_image(i, m.res, m.pixels, m.depth_pixels, m.depth_scale * result.scale, m.mask_pixels, m.maskindex_pixels, m.maskindex_len, m.image_data_on_gpu, m.image_type, EDepthDataType::UShort, sharpen_amount, m.white_transparent, m.black_transparent, m.mask_color, m.rays);
	...
	for (uint32_t i = 0; i < result.n_images; ++i) {
		...
		free(images[i].mask_pixels);
		free(images[i].maskindex_pixels);
	}
	return result;
}


// Set all the images with the same image, image_resolution, mask info
void NerfDataset::set_training_image_all(const Eigen::Vector2i& image_resolution, const void* pixels, const void* depth_pixels, float depth_scale, const void* mask_pixels,const void* maskindex_pixels, uint32_t maskindex_len, bool image_data_on_gpu, EImageDataType image_type, EDepthDataType depth_type, float sharpen_amount, bool white_transparent, bool black_transparent, uint32_t mask_color, const Ray *rays) {
	...
	GPUMemory<float> mask_tmp;
	GPUMemory<uint16_t> maskindex_tmp;

	// Reset all the images: only save for the first one while the others only keep a reference to the first one
	for (uint32_t frame_idx = 0; frame_idx < n_images; frame_idx++) {

		if(frame_idx==0){

			void* dst = nullptr;
			if(image_type_stride>0){
			...
			if (depth_scale >= 0.f) {
				depthmemory[frame_idx].resize(img_size); // Todo: Why use img_size instead of n_pixels?
			...
			if (maskindex_len > 0){
				...
				// copy over mask if provided
				maskmemory[frame_idx].resize(img_size); // Todo: Why use img_size instead of n_pixels?
				float* mask_dst = maskmemory[frame_idx].data();

				if (mask_pixels && !image_data_on_gpu) {
					mask_tmp.resize(n_pixels * image_type_size(EImageDataType::Float));
					mask_tmp.copy_from_host((float*)mask_pixels);
					mask_pixels = mask_tmp.data();
				}

				// Todo: May need double check
				if (image_type != EImageDataType::Byte){
					CUDA_CHECK_THROW(cudaMemcpy(mask_dst, mask_pixels, n_pixels * image_type_size(EImageDataType::Float), image_data_on_gpu ? cudaMemcpyDeviceToDevice : cudaMemcpyHostToDevice));
				}
				else {
					// Mainly used when initializing the dataset
					linear_kernel(copy_mask<float>, 0, nullptr, n_pixels, mask_dst, (const float*)mask_pixels);
				}

				// copy over maskindex if provided
				maskindexmemory[frame_idx].resize(img_size);
				uint16_t* maskindex_dst = maskindexmemory[frame_idx].data();

				if (maskindex_pixels && !image_data_on_gpu) {
					maskindex_tmp.resize(maskindex_len * 4); // 4 for uint32_t
					maskindex_tmp.copy_from_host((uint16_t*)maskindex_pixels);
					maskindex_pixels = maskindex_tmp.data();
				}

				// Todo: May need double check
				if (image_type != EImageDataType::Byte){
					CUDA_CHECK_THROW(cudaMemcpy(maskindex_dst, maskindex_pixels, maskindex_len * 4, image_data_on_gpu ? cudaMemcpyDeviceToDevice : cudaMemcpyHostToDevice));
				}
				else {
					// Mainly used when initializing the dataset
					linear_kernel(copy_maskindex<uint16_t>, 0, nullptr, maskindex_len, maskindex_dst, (const uint16_t*)maskindex_pixels);
				}
			}
			else{
				maskmemory[frame_idx].free_memory();
				maskindexmemory[frame_idx].free_memory();
			}
			...
			metadata[frame_idx].maskindex = maskindexmemory[frame_idx].data();
			metadata[frame_idx].maskindex_len = maskindex_len;
			...
			metadata[frame_idx].pixels = pixelmemory[0].data();
			metadata[frame_idx].depth = depthmemory[0].data();
			metadata[frame_idx].mask = maskmemory[0].data();
			metadata[frame_idx].maskindex = maskindexmemory[0].data();
			metadata[frame_idx].maskindex_len = maskindex_len;
			metadata[frame_idx].resolution = image_resolution;
			metadata[frame_idx].image_data_type = image_type;
			metadata[frame_idx].rays = raymemory[0].data();
			update_metadata(frame_idx, frame_idx + 1);
		}
	}
}
...
void NerfDataset::set_training_image(int frame_idx, const Eigen::Vector2i& image_resolution, const void* pixels, const void* depth_pixels, float depth_scale, const void* mask_pixels,const void* maskindex_pixels, uint32_t maskindex_len, bool image_data_on_gpu, EImageDataType image_type, EDepthDataType depth_type, float sharpen_amount, bool white_transparent, bool black_transparent, uint32_t mask_color, const Ray *rays) {
	...
	if (!image_data_on_gpu && image_type == EImageDataType::Byte) {
		...
		if (mask_pixels) {
			mask_tmp.resize(n_pixels * image_type_size(EImageDataType::Float)); // 4 for float
			mask_tmp.copy_from_host((float*)mask_pixels);
			mask_pixels = mask_tmp.data();
		}
	...
	if (maskindex_len > 0){
		...
		if (maskindex_pixels && !image_data_on_gpu) {
			maskindex_tmp.resize(maskindex_len * 4); // 4 for uint16_t
			maskindex_tmp.copy_from_host((uint16_t*)maskindex_pixels);
			maskindex_pixels = maskindex_tmp.data();
		}
