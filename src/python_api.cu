...
void Testbed::Nerf::Training::set_image_all(pybind11::array_t<float> img, pybind11::array_t<float> depth_img, float depth_scale, pybind11::array_t<float> mask_img, pybind11::array_t<uint16_t> maskindex, int maskindex_len) {
	...
	py::buffer_info mask_buf = mask_img.request();
	py::buffer_info maskindex_buf = maskindex.request();

	dataset.set_training_image_all({img_buf.shape[1], img_buf.shape[0]}, (const void*)img_buf.ptr, (const float*)depth_buf.ptr, depth_scale, (const float*)mask_buf.ptr, (const uint16_t*)maskindex_buf.ptr, maskindex_len, false, EImageDataType::Float, EDepthDataType::Float);
}

void Testbed::Nerf::Training::set_image(int frame_idx, pybind11::array_t<float> img, pybind11::array_t<float> depth_img, float depth_scale, pybind11::array_t<float> mask_img, pybind11::array_t<uint16_t> maskindex, int maskindex_len) {
	...
	dataset.set_training_image(frame_idx, {img_buf.shape[1], img_buf.shape[0]}, (const void*)img_buf.ptr, (const float*)depth_buf.ptr, depth_scale, (const float*)mask_buf.ptr, (const uint16_t*)maskindex_buf.ptr, maskindex_len, false, EImageDataType::Float, EDepthDataType::Float);

}

void Testbed::set_visualization_camera_idx_list(py::array_t<int> idx_list) {

	py::buffer_info idx_list_buf = idx_list.request();

	int * parr;
	parr = (int *)malloc(sizeof(int) * idx_list_buf.shape[0]);
	memset(parr, 0, sizeof(int) * idx_list_buf.shape[0]);
	int *data = static_cast<int *>(idx_list_buf.ptr);

	for (size_t i = 0; i< idx_list_buf.shape[0];i++) {
		parr[i] = data[i];
	}

	visualization_camera_idx_list = parr;
	n_visualization_camera_idx_list = idx_list_buf.shape[0];
}
...
PYBIND11_MODULE(pyngp, m) {
	...
	testbed
		...
		.def_readonly("window_status", &Testbed::m_render_window, "The status of the window.")
	...
	testbed
		...
		.def("set_max_level", &Testbed::set_max_level,
			py::arg("maxlevel") = 1.0f
		)
		...
		.def("reset_training_step", &Testbed::reset_training_step)
		.def("reset_loss", &Testbed::reset_loss)
		...
		// Alex's hack
		.def("grab_gl_pixels", [](py::object &obj) -> py::array_t<uint8_t> {
			Testbed& self = obj.cast<Testbed&>();
			int w=self.m_window_res.x(),h=self.m_window_res.y();
			py::array_t<uint8_t> result({h,w,4});
            py::buffer_info buf = result.request();
            glReadPixels(0, 0, w, h, GL_RGBA, GL_UNSIGNED_BYTE, buf.ptr);
			return result;
		}, "grab the current gl framebuffer")

		.def("find_best_training_view_loss", &Testbed::find_best_training_view_loss,
			"Return the best training view index with the lowest loss."
			)
		.def("get_training_view_loss", &Testbed::get_training_view_loss,
			py::arg("frame_idx"),
			"Return the relative training loss (percent of all the views)."
			)

		.def("set_visualization_camera_idx_list", &Testbed::set_visualization_camera_idx_list,
			py::arg("idx_list")= py::none(),
			"set all the camera index to be visualized."
			)
		;
	...
	nerf
		...
		.def_readwrite("sharpness_threshold", &Testbed::Nerf::sharpness_threshold)
	...
	py::class_<Testbed::Nerf::Training>(nerf, "Training")
		...
		.def_readwrite("train_cam_with_segmented_bg", &Testbed::Nerf::Training::train_cam_with_segmented_bg) // Force to compute loss on the background

		// Some experimental settings, not used in the paper
		.def_readwrite("train_cam_with_mask_loss", &Testbed::Nerf::Training::train_cam_with_mask_loss)
		.def_readwrite("train_cam_with_interest_region", &Testbed::Nerf::Training::train_cam_with_interest_region)
		.def_readwrite("mask_supervision_lambda", &Testbed::Nerf::Training::mask_supervision_lambda)
		.def_readwrite("mask_render_ratio", &Testbed::Nerf::Training::mask_render_ratio) // The ratio to determine if we sample from the mask region or the rendered region

		.def_readwrite("keep_first_pose_as_still", &Testbed::Nerf::Training::keep_first_pose_as_still)
		.def_readwrite("verbose", &Testbed::Nerf::Training::verbose)
		...
		.def_readwrite("extrinsic_learning_rate_base_step", &Testbed::Nerf::Training::extrinsic_learning_rate_base_step)
		...
		.def("set_image", &Testbed::Nerf::Training::set_image,
			...
			py::arg("mask_img")= py::none(),
			py::arg("maskindex")= py::none(),
			py::arg("maskindex_len")= 0,
		...
		.def("set_image_all", &Testbed::Nerf::Training::set_image_all,
			py::arg("img"),
			py::arg("depth_img"),
			py::arg("depth_scale")=1.0f,
			...
			"set all the training images. must be a floating point numpy array of (H,W,C) with 4 channels; linear color space; W and H must match image size of the rest of the dataset"
		)
		;
