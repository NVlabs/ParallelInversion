...
/* Illegal to call memory allocation functions in device code
inline __device__ uint32_t nerf_random_pixel_id_mask_region(default_rng_t& rng, const Vector2i& resolution, bool snap_to_pixel_centers, const float* mask) {

	// x from [0,1) See https://github.com/NVlabs/tiny-cuda-nn/blob/master/include/tiny-cuda-nn/random.h#L50
	float x = random_val(rng);

	// Find all the non-zero indices in mask
	uint32_t n_nonzero = 0;
	uint32_t* nonzero_indices = (uint32_t*)malloc(sizeof(uint32_t) * resolution.x() * resolution.y());
	...
	uint32_t idx = (uint32_t)round(x*(n_nonzero-1));
	// Vector2f xy = {nonzero_indices[idx]% resolution.x(), nonzero_indices[idx]/ resolution.x()};

	return nonzero_indices[idx];
}*/
...
inline __device__ Vector2f projection_to_image(const Matrix<float, 4, 4>&proj, Vector3f p) {
	Vector4f ph;
	ph << p, 1.f;
	Vector4f pa = proj * ph;
...
__global__ void generate_training_samples_nerf(
	...
	bool train_cam_with_segmented_bg,
	bool train_cam_with_interest_region,
	Matrix3f render_aabb_to_local,
	float mask_render_ratio,
	float* __restrict__ sampled_xy,
	...
	// In our case, xy does not matter for xform/read_rgba. We do not have that corner case.
	// We only want to focus on the projected unit square
	// We may need similar stuff as we compute the samples

	if (train_cam_with_interest_region){

		Vector2f xy_new;
		bool is_in_interest_region = false;
		bool proj_bbox_initialized = false;

		Vector2f maxVal_rendered;
		Vector2f minVal_rendered;

		Vector2f maxVal_mask = {(float)metadata[img].maskindex[0], (float)metadata[img].maskindex[1]};
		Vector2f minVal_mask = {(float)metadata[img].maskindex[2], (float)metadata[img].maskindex[3]};

		// Normalize the value to (0,1)
		maxVal_mask.x() = maxVal_mask.x() / (float)metadata[img].resolution.x();
		maxVal_mask.y() = maxVal_mask.y() / (float)metadata[img].resolution.y();

		minVal_mask.x() = minVal_mask.x() / (float)metadata[img].resolution.x();
		minVal_mask.y() = minVal_mask.y() / (float)metadata[img].resolution.y();

		Vector2f maxVal_combined;
		Vector2f minVal_combined;

		while (!is_in_interest_region){

			if (!proj_bbox_initialized){
				xy_new = nerf_random_image_pos_training(rng, resolution, snap_to_pixel_centers, cdf_x_cond_y, cdf_y, cdf_res, img);
			}else{
				// We limit the range of xy to a smaller region (bounding box of mask & rendered target)
				xy_new = random_val_2d(rng);
				xy_new.x() = xy_new.x() * (maxVal_combined.x() - minVal_combined.x()) + minVal_combined.x();
				xy_new.y() = xy_new.y() * (maxVal_combined.y() - minVal_combined.y()) + minVal_combined.y();

				if (snap_to_pixel_centers) {
					xy_new = (xy_new.cwiseProduct(resolution.cast<float>()).cast<int>().cwiseMax(0).cwiseMin(resolution - Vector2i::Ones()).cast<float>() + Vector2f::Constant(0.5f)).cwiseQuotient(resolution.cast<float>());
				}
			}

			if (read_mask(xy_new, resolution, metadata[img].mask) > 0){
				is_in_interest_region = true;
				xy = xy_new;
			} else{

				if (!proj_bbox_initialized){
					
					// See Testbed::draw_visualizations as a reference
					// Project the unit cube vertices onto the image plane
					Matrix<float, 4, 4> world2view, view2world, view2proj, world2proj;
					view2world.setIdentity();
					view2world.block<3,4>(0,0) = xform;

					// See Testbed::calc_focal_length and Testbed::set_camera_to_training_view as a reference
					Vector2f focal = metadata[img].focal_length/ metadata[img].resolution[1];
					float zscale = 1.0f / focal[1];

					float xyscale = (float)metadata[img].resolution[1];
					Vector2f screen_center = {0.5f, 0.5f};
					view2proj <<
						xyscale, 0,       (float)metadata[img].resolution.x()*screen_center.x()*zscale, 0,
						0,       xyscale, (float)metadata[img].resolution.y()*screen_center.y()*zscale, 0,
						0,       0,       1,                                                0,
						0,       0,       zscale,                                           0;
					...
					// See visualize_unit_cube as a reference
					Eigen::Matrix3f m = render_aabb_to_local.transpose();

					// Save the vertices of the unit cube in column major order, 2x8
					Matrix<float, 2, 8> bbox_proj;
					bbox_proj <<
						projection_to_image(world2proj, m * Vector3f{0.f,0.f,0.f}),
						projection_to_image(world2proj, m * Vector3f{1.f,0.f,0.f}),
						projection_to_image(world2proj, m * Vector3f{0.f,1.f,0.f}),
						projection_to_image(world2proj, m * Vector3f{0.f,0.f,1.f}),
						projection_to_image(world2proj, m * Vector3f{1.f,1.f,0.f}),
						projection_to_image(world2proj, m * Vector3f{1.f,0.f,1.f}),
						projection_to_image(world2proj, m * Vector3f{0.f,1.f,1.f}),
						projection_to_image(world2proj, m * Vector3f{1.f,1.f,1.f});

					maxVal_rendered = bbox_proj.rowwise().maxCoeff();
					minVal_rendered = bbox_proj.rowwise().minCoeff();

					// Clamp the projected vertices to the image plane
					maxVal_rendered = maxVal_rendered.cwiseMax(0.f).cwiseMin(metadata[img].resolution.cast<float>() - Vector2f::Ones());
					minVal_rendered = minVal_rendered.cwiseMax(0.f).cwiseMin(metadata[img].resolution.cast<float>() - Vector2f::Ones());

					// Normalize the value to (0,1)
					maxVal_rendered.x() = maxVal_rendered.x() / (float)metadata[img].resolution.x();
					maxVal_rendered.y() = maxVal_rendered.y() / (float)metadata[img].resolution.y();

					minVal_rendered.x() = minVal_rendered.x() / (float)metadata[img].resolution.x();
					minVal_rendered.y() = minVal_rendered.y() / (float)metadata[img].resolution.y();


					// Initialize the combined bounding box
					if (maxVal_rendered.x()>maxVal_mask.x()){
						maxVal_combined.x()=maxVal_rendered.x();
					}
					else{
						maxVal_combined.x()=maxVal_mask.x();
					}

					if (maxVal_rendered.y()>maxVal_mask.y()){
						maxVal_combined.y()=maxVal_rendered.y();
					}
					else{
						maxVal_combined.y()=maxVal_mask.y();
					}

					if (minVal_rendered.x()<minVal_mask.x()){
						minVal_combined.x()=minVal_rendered.x();
					}
					else{
						minVal_combined.x()=minVal_mask.x();
					}

					if (minVal_rendered.y()<minVal_mask.y()){
						minVal_combined.y()=minVal_rendered.y();
					}
					else{
						minVal_combined.y()=minVal_mask.y();
					}

					proj_bbox_initialized = true;
				}

				if (maxVal_rendered.x() <= minVal_rendered.x() || maxVal_rendered.y() <= minVal_rendered.y()) {
					continue;
				}

				if ( xy_new.x() <= maxVal_rendered.x() && xy_new.x() >= minVal_rendered.x() && xy_new.y() <= maxVal_rendered.y() && xy_new.y() >= minVal_rendered.y() ) {
					is_in_interest_region = true;
					xy = xy_new;
				}
	...
	pix_idx = pixel_idx(xy, resolution, 0);
	...
	if (read_rgba(xy, resolution, metadata[img].pixels, metadata[img].image_data_type).x() < 0.0f) {
		// Note They only work if we deliberately mask away some pixels. Usually, it does not happen.
		return;
	}
	...
	// In our case, we still want to compute the gradient if the ray is looking at the background.
	if (!train_cam_with_segmented_bg){
			if (j == 0 && !train_envmap) {
				return;
			}
		}
	...
	sampled_xy[ray_idx*2+0] = xy.x();
	sampled_xy[ray_idx*2+1] = xy.y();
...
__global__ void compute_loss_kernel_train_nerf(
	...
	float* __restrict__ loss_rgb_output,
	float* __restrict__ loss_mask_output,
	...
	bool train_cam_with_segmented_bg,
	bool train_cam_with_mask_loss,
	bool train_cam_with_interest_region,
	Matrix3f render_aabb_to_local,
	float mask_supervision_lambda,
	float mask_render_ratio,
	float* __restrict__ sampled_xy,

	float* __restrict__ error_map,

	float* __restrict__ error_map_rgb,
	float* __restrict__ error_map_mask,
	...
	if (train_cam_with_interest_region) {
		xy = {sampled_xy[i*2+0], sampled_xy[i*2+1]};
	}
	...
	if (train_in_linear_colors || color_space == EColorSpace::Linear) {
		...
		masktarget = texsamp.w();

		// Note the default setting here according to https://github.com/NVlabs/instant-ngp/discussions/227
	...
	// Note in our case, we may have T=1, numsteps=0 since there is no density along the ray
	...
	if (compacted_numsteps == 0) {
		...
		// In our case, we still want to compute loss and others if there is no sample.
	}
	...
	// Use L2 loss for now
	float mask_loss_gradient = (mask_ray - masktarget) * 2.f * mask_supervision_lambda;
	...
	// Note use 3 channels so need mean operation
	float mean_loss = 0.0f;
	if (masktarget > 0.0f){
		mean_loss = lg.loss.mean();
	}

	float rgb_loss = mean_loss;
	float mask_loss = 0.0f;

	if (train_cam_with_mask_loss && masktarget > 0.0f) {
		mask_loss = (mask_ray - masktarget) * (mask_ray - masktarget) * mask_supervision_lambda;
		mean_loss += mask_loss;
	}

	// Note it is displayed in the loss graph (the average loss/ray across all the views)
	if (loss_output) {
		loss_output[i] = mean_loss / (float)n_rays;

		if (train_cam_with_mask_loss) {
			loss_rgb_output[i] = rgb_loss / (float)n_rays;
			loss_mask_output[i] = mask_loss / (float)n_rays;
		}
	}

	// To record training loss
	if (error_map) {
		...
		auto deposit_val = [&](int x, int y, float val, float* __restrict__ error_map_temp) {
			atomicAdd(&error_map_temp[img * error_map_res.prod() + y * error_map_res.x() + x], val);
		};
		...
		if (sharpness_data && aabb.contains(hitpoint)) {
			...
			if (train_cam_with_mask_loss) {
				rgb_loss *= fmaxf(sharp / grid_sharp, 0.01f);
				mask_loss *= fmaxf(sharp / grid_sharp, 0.01f);
			}
		}

		// float sample_sum = (1 - weight.x()) * (1 - weight.y()) * mean_loss;

		deposit_val(idx.x(),   idx.y(),   (1 - weight.x()) * (1 - weight.y()) * mean_loss, error_map);
		deposit_val(idx.x()+1, idx.y(),        weight.x()  * (1 - weight.y()) * mean_loss, error_map);
		deposit_val(idx.x(),   idx.y()+1, (1 - weight.x()) *      weight.y()  * mean_loss, error_map);
		deposit_val(idx.x()+1, idx.y()+1,      weight.x()  *      weight.y()  * mean_loss, error_map);

		if (train_cam_with_mask_loss){

			deposit_val(idx.x(),   idx.y(),   (1 - weight.x()) * (1 - weight.y()) * rgb_loss, error_map_rgb);
			deposit_val(idx.x()+1, idx.y(),        weight.x()  * (1 - weight.y()) * rgb_loss, error_map_rgb);
			deposit_val(idx.x(),   idx.y()+1, (1 - weight.x()) *      weight.y()  * rgb_loss, error_map_rgb);
			deposit_val(idx.x()+1, idx.y()+1,      weight.x()  *      weight.y()  * rgb_loss, error_map_rgb);

			deposit_val(idx.x(),   idx.y(),   (1 - weight.x()) * (1 - weight.y()) * mask_loss, error_map_mask);
			deposit_val(idx.x()+1, idx.y(),        weight.x()  * (1 - weight.y()) * mask_loss, error_map_mask);
			deposit_val(idx.x(),   idx.y()+1, (1 - weight.x()) *      weight.y()  * mask_loss, error_map_mask);
			deposit_val(idx.x()+1, idx.y()+1,      weight.x()  *      weight.y()  * mask_loss, error_map_mask);
		}
	...
	for (uint32_t j = 0; j < compacted_numsteps; ++j) {
		...
		// We want to have a similar gradient for the alpha as the depth gradient
		const float mask_suffix = mask_ray - mask_ray2;
		float mask_supervision = mask_loss_gradient * (T - mask_suffix);
		...
		if (!train_cam_with_mask_loss){
			mask_supervision = 0.f;
		}

		float dloss_by_dmlp = density_derivative * (
			dt * (lg.gradient.matrix().dot((T * rgb - suffix).matrix()) + depth_supervision + mask_supervision)
		);
...
__global__ void compute_cam_gradient_train_nerf(
	...
	// Based on my understanding, the following code is not important to us if we do not enable distortion_gradient. The cam gradient is based on the samples & losses.
...
/* This is the original implementation for only one error map.
...
__global__ void construct_cdf_2d(
	...
	const float* __restrict__ data_rgb,
	float* __restrict__ cdf_x_cond_y_rgb,
	float* __restrict__ cdf_y_rgb,

	const float* __restrict__ data_mask,
	float* __restrict__ cdf_x_cond_y_mask,
	float* __restrict__ cdf_y_mask
) {
	const uint32_t y = threadIdx.x + blockIdx.x * blockDim.x;
	const uint32_t img = threadIdx.y + blockIdx.y * blockDim.y;
	if (y >= height || img >= n_images) return;
	...
	// For rgb
	data_rgb += offset_xy;
	cdf_x_cond_y_rgb += offset_xy;

	float cum_rgb = 0;
	for (uint32_t x = 0; x < width; ++x) {
		cum_rgb += data_rgb[x] + 1e-10f;
		cdf_x_cond_y_rgb[x] = cum_rgb;
	}

	cdf_y_rgb[img * height + y] = cum_rgb;
	float norm_rgb = __frcp_rn(cum_rgb);

	for (uint32_t x = 0; x < width; ++x) {
		cdf_x_cond_y_rgb[x] = (1.0f - MIN_PDF) * cdf_x_cond_y_rgb[x] * norm_rgb + MIN_PDF * (float)(x+1) / (float)width;
	}

	// For mask
	data_mask += offset_xy;
	cdf_x_cond_y_mask += offset_xy;

	float cum_mask = 0;
	for (uint32_t x = 0; x < width; ++x) {
		cum_mask += data_mask[x] + 1e-10f;
		cdf_x_cond_y_mask[x] = cum_mask;
	}

	cdf_y_mask[img * height + y] = cum_mask;
	float norm_mask = __frcp_rn(cum_mask);

	for (uint32_t x = 0; x < width; ++x) {
		cdf_x_cond_y_mask[x] = (1.0f - MIN_PDF) * cdf_x_cond_y_mask[x] * norm_mask + MIN_PDF * (float)(x+1) / (float)width;
	}
}
...
__global__ void construct_cdf_1d(
	...
	float* __restrict__ cdf_y_rgb,
	float* __restrict__ cdf_img_rgb,

	float* __restrict__ cdf_y_mask,
	float* __restrict__ cdf_img_mask
) {
	const uint32_t img = threadIdx.x + blockIdx.x * blockDim.x;
	if (img >= n_images) return;
	...
	cdf_img_rgb[img] = cum_rgb;

	float norm_rgb = __frcp_rn(cum_rgb);
	for (uint32_t y = 0; y < height; ++y) {
		cdf_y_rgb[y] = (1.0f - MIN_PDF) * cdf_y_rgb[y] * norm + MIN_PDF * (float)(y+1) / (float)height;
	}

	// For mask
	cdf_y_mask += img * height;

	float cum_mask = 0;
	for (uint32_t y = 0; y < height; ++y) {
		cum_mask += cdf_y_mask[y];
		cdf_y_mask[y] = cum_mask;
	}

	cdf_img_mask[img] = cum_mask;

	float norm_mask = __frcp_rn(cum_mask);
	for (uint32_t y = 0; y < height; ++y) {
		cdf_y_mask[y] = (1.0f - MIN_PDF) * cdf_y_mask[y] * norm + MIN_PDF * (float)(y+1) / (float)height;
	}
}
...
void Testbed::load_nerf() {
	if (!m_data_path.empty()) {
		...
		m_nerf.training.dataset = ngp::load_nerf(json_paths, m_nerf.sharpness_threshold, m_nerf.sharpen);
	}
...
void Testbed::Nerf::Training::Counters::prepare_for_training_steps(cudaStream_t stream) {
	...
	loss_rgb.enlarge(rays_per_batch);
	// CUDA_CHECK_THROW(cudaMemsetAsync(numsteps_counter.data(), 0, sizeof(uint32_t), stream)); // clear the counter in the first slot
	// CUDA_CHECK_THROW(cudaMemsetAsync(numsteps_counter_compacted.data(), 0, sizeof(uint32_t), stream)); // clear the counter in the first slot
	CUDA_CHECK_THROW(cudaMemsetAsync(loss_rgb.data(), 0, sizeof(float)*rays_per_batch, stream));

	loss_mask.enlarge(rays_per_batch);
	// CUDA_CHECK_THROW(cudaMemsetAsync(numsteps_counter.data(), 0, sizeof(uint32_t), stream)); // clear the counter in the first slot
	// CUDA_CHECK_THROW(cudaMemsetAsync(numsteps_counter_compacted.data(), 0, sizeof(uint32_t), stream)); // clear the counter in the first slot
	CUDA_CHECK_THROW(cudaMemsetAsync(loss_mask.data(), 0, sizeof(float)*rays_per_batch, stream));
}

void Testbed::Nerf::Training::Counters::update_after_training(float& loss_scalar, float& loss_rgb_scalar, float& loss_mask_scalar, uint32_t target_batch_size, bool get_loss_scalar, cudaStream_t stream) {
	...
	if (get_loss_scalar) {
		...
		//tlog::info() << "loss_scalar: " << (float)loss_scalar;
		loss_rgb_scalar = reduce_sum(loss_rgb.data(), rays_per_batch, stream) * (float)measured_batch_size / (float)target_batch_size;
		loss_mask_scalar = reduce_sum(loss_mask.data(), rays_per_batch, stream) * (float)measured_batch_size / (float)target_batch_size;
	}

	//tlog::info() << "rays_per_batch: " << (float)rays_per_batch;
	//tlog::info() << "measured_batch_size: " << (float)measured_batch_size;
	//tlog::info() << "target_batch_size: " << (float)target_batch_size;

	rays_per_batch = (uint32_t)((float)rays_per_batch * (float)target_batch_size / (float)measured_batch_size);

	// We may want to increase the upper bound here
...
void Testbed::train_nerf(uint32_t target_batch_size, bool get_loss_scalar, cudaStream_t stream) {
	...
	if (m_nerf.training.n_steps_since_error_map_update == 0 && !m_nerf.training.dataset.metadata.empty()) {
		...
		if (m_nerf.training.verbose){
			tlog::info() << "Reinitialize error_map";
		}

		Eigen::Vector2i res = m_nerf.training.dataset.metadata[0].resolution;

		// min(..., res), the max is set to be the resolution of the training image. n_samples_per_image is only used here.
		...
		// Initialize the error map to 0
		CUDA_CHECK_THROW(cudaMemsetAsync(m_nerf.training.error_map.data.data(), 0, m_nerf.training.error_map.data.get_bytes(), stream));

		m_nerf.training.error_map_rgb.data.resize(m_nerf.training.error_map.resolution.prod() * m_nerf.training.dataset.n_images);
		CUDA_CHECK_THROW(cudaMemsetAsync(m_nerf.training.error_map_rgb.data.data(), 0, m_nerf.training.error_map_rgb.data.get_bytes(), stream));
		m_nerf.training.error_map_mask.data.resize(m_nerf.training.error_map.resolution.prod() * m_nerf.training.dataset.n_images);
		CUDA_CHECK_THROW(cudaMemsetAsync(m_nerf.training.error_map_mask.data.data(), 0, m_nerf.training.error_map_mask.data.get_bytes(), stream));
	}
	...
	train_nerf_step(
		...
		m_nerf.training.counters_rgb.loss_rgb.data(),
		m_nerf.training.counters_rgb.loss_mask.data(),
	...
	float loss_rgb_scalar = 0.0f;
	float loss_mask_scalar = 0.0f;
	m_nerf.training.counters_rgb.update_after_training(loss_scalar, loss_rgb_scalar, loss_mask_scalar, target_batch_size, get_loss_scalar, stream);

	bool zero_records = m_nerf.training.counters_rgb.measured_batch_size == 0;
	if (get_loss_scalar) {
		m_loss_scalar.update(loss_scalar);

		m_loss_rgb_scalar.update(loss_rgb_scalar);
		m_loss_mask_scalar.update(loss_mask_scalar);
	}

	if (zero_records) {
		m_loss_scalar.set(0.f);

		m_loss_rgb_scalar.set(0.f);
		m_loss_mask_scalar.set(0.f);
	...
	if (accumulate_error && m_nerf.training.n_steps_since_error_map_update >= m_nerf.training.n_steps_between_error_map_updates) {
		...
		m_nerf.training.error_map_rgb.cdf_x_cond_y.resize(m_nerf.training.error_map.cdf_resolution.prod() * m_nerf.training.dataset.n_images);
		m_nerf.training.error_map_rgb.cdf_y.resize(m_nerf.training.error_map.cdf_resolution.y() * m_nerf.training.dataset.n_images);
		m_nerf.training.error_map_rgb.cdf_img.resize(m_nerf.training.dataset.n_images);

		m_nerf.training.error_map_mask.cdf_x_cond_y.resize(m_nerf.training.error_map.cdf_resolution.prod() * m_nerf.training.dataset.n_images);
		m_nerf.training.error_map_mask.cdf_y.resize(m_nerf.training.error_map.cdf_resolution.y() * m_nerf.training.dataset.n_images);
		m_nerf.training.error_map_mask.cdf_img.resize(m_nerf.training.dataset.n_images);

		// Initialize cdf_x_cond_y/cdf_y/cdf_img
		...
		CUDA_CHECK_THROW(cudaMemsetAsync(m_nerf.training.error_map_rgb.cdf_x_cond_y.data(), 0, m_nerf.training.error_map_rgb.cdf_x_cond_y.get_bytes(), stream));
		CUDA_CHECK_THROW(cudaMemsetAsync(m_nerf.training.error_map_rgb.cdf_y.data(), 0, m_nerf.training.error_map_rgb.cdf_y.get_bytes(), stream));
		CUDA_CHECK_THROW(cudaMemsetAsync(m_nerf.training.error_map_rgb.cdf_img.data(), 0, m_nerf.training.error_map_rgb.cdf_img.get_bytes(), stream));

		CUDA_CHECK_THROW(cudaMemsetAsync(m_nerf.training.error_map_mask.cdf_x_cond_y.data(), 0, m_nerf.training.error_map_mask.cdf_x_cond_y.get_bytes(), stream));
		CUDA_CHECK_THROW(cudaMemsetAsync(m_nerf.training.error_map_mask.cdf_y.data(), 0, m_nerf.training.error_map_mask.cdf_y.get_bytes(), stream));
		CUDA_CHECK_THROW(cudaMemsetAsync(m_nerf.training.error_map_mask.cdf_img.data(), 0, m_nerf.training.error_map_mask.cdf_img.get_bytes(), stream));
		...
		*/
		...
		construct_cdf_2d<<<blocks, threads, 0, stream>>>(
			...
			m_nerf.training.error_map_rgb.data.data(),
			m_nerf.training.error_map_rgb.cdf_x_cond_y.data(),
			m_nerf.training.error_map_rgb.cdf_y.data(),

			m_nerf.training.error_map_mask.data.data(),
			m_nerf.training.error_map_mask.cdf_x_cond_y.data(),
			m_nerf.training.error_map_mask.cdf_y.data()
		);
		linear_kernel(construct_cdf_1d, 0, stream,
			m_nerf.training.dataset.n_images,
			m_nerf.training.error_map.cdf_resolution.y(),
			...
			m_nerf.training.error_map_rgb.cdf_y.data(),
			m_nerf.training.error_map_rgb.cdf_img.data(),

			m_nerf.training.error_map_mask.cdf_y.data(),
			m_nerf.training.error_map_mask.cdf_img.data()
		);
		...
		m_nerf.training.error_map.abs_img_cpu.resize(m_nerf.training.error_map.cdf_img.size());
		...
		for (size_t i = 0; i < cdf_img_cpu.size(); ++i) {
			...
			m_nerf.training.error_map.abs_img_cpu[i] = m_nerf.training.error_map.pmf_img_cpu[i] * m_loss_scalar.val() * m_nerf.training.counters_rgb.rays_per_batch;
		...
		// The original code is to update the error map slower and slower. We do not want that. As we need to query from it with some fixed steps.
		// m_nerf.training.n_steps_between_error_map_updates = (uint32_t)(m_nerf.training.n_steps_between_error_map_updates * 1.5f);

		if (m_nerf.training.train_cam_with_segmented_bg){

			// For rgb
			// Compute image CDF on the CPU. It's single-threaded anyway. No use parallelizing.
			m_nerf.training.error_map_rgb.pmf_img_cpu.resize(m_nerf.training.error_map_rgb.cdf_img.size());
			m_nerf.training.error_map_rgb.abs_img_cpu.resize(m_nerf.training.error_map_rgb.cdf_img.size());

			m_nerf.training.error_map_rgb.cdf_img.copy_to_host(m_nerf.training.error_map_rgb.pmf_img_cpu);
			std::vector<float> cdf_img_cpu_rgb = m_nerf.training.error_map_rgb.pmf_img_cpu; // Copy unnormalized PDF into CDF buffer
			float cum_rgb = 0;
			for (float& f : cdf_img_cpu_rgb) {
				cum_rgb += f;
				f = cum_rgb;
			}

			norm = 1.0f / cum_rgb;
			for (size_t i = 0; i < cdf_img_cpu_rgb.size(); ++i) {
				constexpr float MIN_PMF = 0.1f;
				m_nerf.training.error_map_rgb.pmf_img_cpu[i] = (1.0f - MIN_PMF) * m_nerf.training.error_map_rgb.pmf_img_cpu[i] * norm + MIN_PMF / (float)m_nerf.training.dataset.n_images;

				// Todo: Not sure if this is the right way to do this.
				m_nerf.training.error_map_rgb.abs_img_cpu[i] = m_nerf.training.error_map_rgb.pmf_img_cpu[i] * m_loss_rgb_scalar.val() * m_nerf.training.counters_rgb.rays_per_batch;

				cdf_img_cpu_rgb[i] = (1.0f - MIN_PMF) * cdf_img_cpu_rgb[i] * norm + MIN_PMF * (float)(i+1) / (float)m_nerf.training.dataset.n_images;
			}
			m_nerf.training.error_map_rgb.cdf_img.copy_from_host(cdf_img_cpu_rgb);

			m_nerf.training.error_map_rgb.is_cdf_valid = true;

			// For mask
			// Compute image CDF on the CPU. It's single-threaded anyway. No use parallelizing.
			m_nerf.training.error_map_mask.pmf_img_cpu.resize(m_nerf.training.error_map_mask.cdf_img.size());
			m_nerf.training.error_map_mask.abs_img_cpu.resize(m_nerf.training.error_map_mask.cdf_img.size());

			m_nerf.training.error_map_mask.cdf_img.copy_to_host(m_nerf.training.error_map_mask.pmf_img_cpu);
			cdf_img_cpu = m_nerf.training.error_map_mask.pmf_img_cpu; // Copy unnormalized PDF into CDF buffer
			cum = 0;
			for (float& f : cdf_img_cpu) {
				cum += f;
				f = cum;
			...
			for (size_t i = 0; i < cdf_img_cpu.size(); ++i) {
				constexpr float MIN_PMF = 0.1f;
				m_nerf.training.error_map_mask.pmf_img_cpu[i] = (1.0f - MIN_PMF) * m_nerf.training.error_map_mask.pmf_img_cpu[i] * norm + MIN_PMF / (float)m_nerf.training.dataset.n_images;

				// Todo: Not sure if this is the right way to do this.
				m_nerf.training.error_map_mask.abs_img_cpu[i] = m_nerf.training.error_map_mask.pmf_img_cpu[i] * m_loss_mask_scalar.val() * m_nerf.training.counters_rgb.rays_per_batch;

				cdf_img_cpu[i] = (1.0f - MIN_PMF) * cdf_img_cpu[i] * norm + MIN_PMF * (float)(i+1) / (float)m_nerf.training.dataset.n_images;
			}
			m_nerf.training.error_map_mask.cdf_img.copy_from_host(cdf_img_cpu);

			m_nerf.training.error_map_mask.is_cdf_valid = true;


			// Resample camera guesses
			// TODO
		}
	}
	...
	if (train_camera && m_nerf.training.n_steps_since_cam_update >= m_nerf.training.n_steps_between_cam_updates) {
		...
		if (m_nerf.training.optimize_extrinsics) {
			...
			// tlog::info() << "cam_pos_gradient: "<< m_nerf.training.cam_pos_gradient[1];
			// tlog::info() << "cam_rot_gradient: "<< m_nerf.training.cam_rot_gradient[1];

			// Optimization step
			for (uint32_t i = 0; i < m_nerf.training.n_images_for_training; ++i) {

				// per_camera_loss_scale = n_images_for_training/128/16
				...
				// used some self-defined functions here from adam_optimizer.h. variable represents the norm value
				...
				// Related to lr decay
				m_nerf.training.cam_pos_offset[i].set_learning_rate(std::max(m_nerf.training.extrinsic_learning_rate * std::pow(0.33f, (float)(m_nerf.training.cam_pos_offset[i].step() / m_nerf.training.extrinsic_learning_rate_base_step)), m_optimizer->learning_rate()/1000.0f));
				m_nerf.training.cam_rot_offset[i].set_learning_rate(std::max(m_nerf.training.extrinsic_learning_rate_rot * std::pow(0.33f, (float)(m_nerf.training.cam_rot_offset[i].step() / m_nerf.training.extrinsic_learning_rate_base_step)), m_optimizer->learning_rate()/1000.0f));

				m_nerf.training.cam_pos_offset[i].step(pos_gradient);
				m_nerf.training.cam_rot_offset[i].step(rot_gradient);

				// Print learning rate
				if (m_nerf.training.verbose && i==0){
					tlog::info() << "current step for cam: "<< m_nerf.training.cam_rot_offset[i].step()<< "  lr: "<<std::max(m_nerf.training.extrinsic_learning_rate_rot * std::pow(0.33f, (float)(m_nerf.training.cam_rot_offset[i].step() / m_nerf.training.extrinsic_learning_rate_base_step)), m_optimizer->learning_rate()/1000.0f);
				}

			}

			if (m_nerf.training.keep_first_pose_as_still){
				m_nerf.training.update_transforms(1, -1);
			}
			else {
				m_nerf.training.update_transforms();
			}
...
void Testbed::train_nerf_step(uint32_t target_batch_size, uint32_t n_rays_per_batch, uint32_t* counter, uint32_t* compacted_counter, float* loss, float* loss_rgb, float* loss_mask, cudaStream_t stream) {
	...
	>(
		...
		target_batch_size,
		1,
		n_rays_per_batch * 2//  Similar to numsteps (one for x, the other for y)
	);
	...
	float* sampled_xy = std::get<11>(scratch);
	...
	linear_kernel(generate_training_samples_nerf, 0, stream,
		...
		m_nerf.training.train_cam_with_segmented_bg,
		m_nerf.training.train_cam_with_interest_region,
		m_nerf.training.dataset.render_aabb_to_local,
		m_nerf.training.mask_render_ratio,
		sampled_xy,
	...
	linear_kernel(compute_loss_kernel_train_nerf, 0, stream,
		...
		m_nerf.training.train_cam_with_segmented_bg,
		m_nerf.training.train_cam_with_mask_loss,
		m_nerf.training.train_cam_with_interest_region,
		m_nerf.training.dataset.render_aabb_to_local,
		m_nerf.training.mask_supervision_lambda,
		m_nerf.training.mask_render_ratio,
		sampled_xy,

		accumulate_error ? m_nerf.training.error_map.data.data() : nullptr,

		accumulate_error ? m_nerf.training.error_map_rgb.data.data() : nullptr,
		accumulate_error ? m_nerf.training.error_map_mask.data.data() : nullptr,
...
int Testbed::find_best_training_view_loss() {
	int bestimage = -1;
	float bestscore = 1000.f;
	for (int i = 0; i < m_nerf.training.n_images_for_training; ++i) {
		float score = m_nerf.training.error_map.pmf_img_cpu[i];
		if (score < bestscore) {
			bestscore = score;
			bestimage = i;
		}
...
float Testbed::get_training_view_loss(int frame_idx) {
	return m_nerf.training.error_map.pmf_img_cpu[frame_idx];
}
