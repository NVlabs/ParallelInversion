...
void add_debug_circle_filled(ImDrawList* list, const Matrix<float, 4, 4>& proj, Vector3f a, uint32_t col, float thickness) {
	ImVec2 aa;
	if (DebugProject(proj, a, aa)) {
		list->AddCircleFilled(aa, 5.f, col, thickness);
	}
}


Vector2f projection_to_image_debug(const Matrix<float, 4, 4>&proj, Vector3f p) {
	Vector4f ph;
	ph << p, 1.f;
	Vector4f pa = proj * ph;
