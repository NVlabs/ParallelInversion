...
public:
	...
	void visualize_nerf_cameras(ImDrawList* list, const Eigen::Matrix<float, 4, 4>& world2proj, int* camera_idx_list);
	...
	void train_nerf_step(uint32_t target_batch_size, uint32_t n_rays_per_batch, uint32_t* counter, uint32_t* compacted_counter, float* loss, float* loss_rgb, float* loss_mask, cudaStream_t stream);
	...
	void set_visualization_camera_idx_list(pybind11::array_t<int> camera_idx_list);
#endif
	...
	void draw_visualizations(ImDrawList* list, const Eigen::Matrix<float, 3, 4>& camera_matrix, int* camera_idx_list);

	int* visualization_camera_idx_list = nullptr;
	int n_visualization_camera_idx_list = 0;
	...
	int find_best_training_view_loss();
	float get_training_view_loss(int frame_idx);
	...
	struct Nerf {
		...
		struct Training {
			...
			struct ErrorMap {
				...
				std::vector<float> abs_img_cpu;
				Eigen::Vector2i resolution = {16, 16};
				Eigen::Vector2i cdf_resolution = {16, 16};
				bool is_cdf_valid = false;
			} error_map;

			ErrorMap error_map_rgb;
			ErrorMap error_map_mask;

			// the updated camera pose (c2w)
			...
			int extrinsic_learning_rate_base_step = 128;
			...
			struct Counters {
				...
				tcnn::GPUMemory<float> loss_rgb;
				tcnn::GPUMemory<float> loss_mask;
				...
				void update_after_training(float& loss_scalar, float& loss_rgb_scalar, float& loss_mask_scalar, uint32_t target_batch_size, bool get_loss_scalar, cudaStream_t stream);
			};
			...
			bool train_envmap = false;
 			...
 			bool train_cam_with_segmented_bg = false;
			bool train_cam_with_mask_loss = false;
			bool train_cam_with_interest_region = false;
			float mask_supervision_lambda = 0.f;
			float mask_render_ratio = 0.5f;
			bool keep_first_pose_as_still = false;
			...
			// void set_image(int frame_idx, pybind11::array_t<float> img, pybind11::array_t<float> depth_img, float depth_scale);
 			...
 			void set_image(int frame_idx, pybind11::array_t<float> img, pybind11::array_t<float> depth_img, float depth_scale, pybind11::array_t<float> mask_img, pybind11::array_t<uint16_t> maskindex, int maskindex_len);
			void set_image_all(pybind11::array_t<float> img, pybind11::array_t<float> depth_img, float depth_scale, pybind11::array_t<float> mask_img, pybind11::array_t<uint16_t> maskindex, int maskindex_len);
	...
	Ema m_loss_scalar = {EEmaType::Time, 100};
 	...
 	Ema m_loss_rgb_scalar = {EEmaType::Time, 100};
	Ema m_loss_mask_scalar = {EEmaType::Time, 100};
