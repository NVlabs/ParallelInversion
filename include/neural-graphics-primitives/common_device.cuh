...
inline NGP_HOST_DEVICE float read_mask(Eigen::Vector2f pos, const Eigen::Vector2i& resolution, const float* mask, uint32_t img = 0) {
	auto read_val = [&](const Eigen::Vector2i& p) {
...
inline NGP_HOST_DEVICE uint16_t read_maskindex(uint32_t pos, const uint16_t* maskindex, uint32_t img = 0) {
	auto read_val = [&](const uint32_t& p) {
		return maskindex[p];
	};
