...
struct TrainingImageMetadata {
	...
	const uint16_t* maskindex = nullptr;
...
struct NerfDataset {
	...
	std::vector<tcnn::GPUMemory<uint16_t>> maskindexmemory;
	...
	void set_training_image(int frame_idx, const Eigen::Vector2i& image_resolution, const void* pixels, const void* depth_pixels, float depth_scale, const void* mask_pixels, const void* maskindex_pixels, const uint32_t maskindex_len, bool image_data_on_gpu, EImageDataType image_type, EDepthDataType depth_type, float sharpen_amount = 0.f, bool white_transparent = false, bool black_transparent = false, uint32_t mask_color = 0, const Ray *rays = nullptr);

	void set_training_image_all(const Eigen::Vector2i& image_resolution, const void* pixels, const void* depth_pixels, float depth_scale, const void* mask_pixels, const void* maskindex_pixels, const uint32_t maskindex_len, bool image_data_on_gpu, EImageDataType image_type, EDepthDataType depth_type, float sharpen_amount = 0.f, bool white_transparent = false, bool black_transparent = false, uint32_t mask_color = 0, const Ray *rays = nullptr);
...
NerfDataset load_nerf(const std::vector<filesystem::path>& jsonpaths, float sharpness_threshold = 0.f, float sharpen_amount = 0.f);
